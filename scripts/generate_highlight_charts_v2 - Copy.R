...
chart_threshold_by_year <- function(df) {
  ...
  p <- ggplot(yearly %>% filter(year>=2017), aes(x = year, y = countries_meeting)) +
...
chart_abuja_countries_trend <- function(df) {
  ...
  p <- ggplot(yearly %>%  filter(year>=2017), aes(x = year, y = countries_meeting, fill = highlight)) +
    geom_bar(stat = "identity", color = "white") +
...
chart_gap_by_subregion <- function(df) {
  ...
  p <- ggplot(subregion_gap, aes(x = Subregion, y = avg_gap, fill = Subregion)) +
    ...
    labs(
      ...
      subtitle = "Middle Africa has the largest gap ($179.2); Eastern Africa has the smallest ($109.6)",
